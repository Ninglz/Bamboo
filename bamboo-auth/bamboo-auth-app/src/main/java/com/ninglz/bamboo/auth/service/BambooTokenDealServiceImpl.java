package com.ninglz.bamboo.auth.service;
...
import com.ninglz.bamboo.common.core.util.SpringContextHolder;
...
public class BambooTokenDealServiceImpl {
	 ...
	 * @return R
	 */
	public StatusResponse removeToken(String token) {

		OAuth2AccessToken accessToken = tokenStore.readAccessToken(token);
		if (accessToken == null || StrUtil.isBlank(accessToken.getValue())) {
			return StatusResponse.error(110, "退出失败，token 无效");
		}
	 ...
	 * @return R
	 */
	public StatusResponse queryTokenByUsername(Page page, String username) {
		String key = keyStrResolver.extract(
				SecurityConstants.BAMBOO_PREFIX + SecurityConstants.OAUTH_PREFIX + "uname_to_access_z:", StrUtil.COLON);
	 ...
	 * @return
	 */
	public StatusResponse queryToken(Page page) {
		// 根据分页参数获取对应数据
		String key = keyStrResolver.extract(SecurityConstants.BAMBOO_PREFIX + SecurityConstants.OAUTH_PREFIX + "access:*",
				StrUtil.COLON);
